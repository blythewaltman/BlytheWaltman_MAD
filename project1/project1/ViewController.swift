...
class ViewController: UIViewController {
    ...
    @IBAction func nextButtonTapped(_ sender: Any) {
        if(stepNumber == 0){ //If the step number is 0 then we are displaying the Ingredients & Tools and want to move on to step 1
            ...
            let alert = UIAlertController(title:"Error: No Further Steps", message: "Press Prev To Go Back", preferredStyle: UIAlertController.Style.alert)
            //create a UIAlertAction object for the button
            let cancelAction = UIAlertAction(title: "Close", style:UIAlertAction.Style.cancel, handler: nil)
            alert.addAction(cancelAction)
            present(alert,animated: true, completion: nil)
    ...
    @IBAction func prevButtonTapped(_ sender: Any) {
        if(stepNumber == 0){
            //create a UIAlertController object
            let alert = UIAlertController(title:"Error: No Previous Steps", message: "Press Next To Continue", preferredStyle: UIAlertController.Style.alert)
            //create a UIAlertAction object for the button
            let cancelAction = UIAlertAction(title: "Close", style:UIAlertAction.Style.cancel, handler: nil)
            alert.addAction(cancelAction)
            present(alert,animated: true, completion: nil)
